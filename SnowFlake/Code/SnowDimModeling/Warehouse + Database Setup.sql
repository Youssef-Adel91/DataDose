...
BEGIN
    ...
    -- ---- 6a. Upsert DIM_DRUG ----
    -- DISTINCT is enough here since DRUG_NAME is the only column pulled from the
    -- source, but QUALIFY-style dedupe is used for DIM_PHARMACY below where a
    -- non-key column (CITY) is also involved.
    ...
    -- QUALIFY picks a single deterministic row per PHARMACY_ID (most recent by
    -- LOAD_TIMESTAMP) - MERGE fails with STATEMENT_ERROR "Duplicate row detected"
    -- if the source side has more than one row per match key, which happens
    -- whenever the same pharmacy appears more than once in a batch.
    MERGE INTO DW.DIM_PHARMACY tgt
    USING (
        SELECT PHARMACY AS PHARMACY_ID, CITY
        FROM STAGING.STG_TRANSACTION
        WHERE IS_PROCESSED = 'FALSE' AND PHARMACY IS NOT NULL AND PHARMACY <> ''
        QUALIFY ROW_NUMBER() OVER (
            PARTITION BY PHARMACY ORDER BY LOAD_TIMESTAMP DESC
        ) = 1
    ) src
    ...
    -- ---- 6d. Mark ONLY rows that are now represented in the fact table as processed.
    --         (Previously this blanket-updated every FALSE row regardless of whether
    --         it was actually inserted — rows skipped by the NOT EXISTS/dedupe check
    --         above, e.g. duplicate TX_IDs, got marked TRUE anyway and were lost
    --         forever. This version only flips rows whose TX_ID now exists in the
    --         fact table, OR whose TX_ID was already a genuine pre-existing duplicate
    --         -- either way keeping this idempotent and safe to rerun.)
...
SELECT COUNT(*) FROM DW.FACT_PRESCRIPTION_EVENT;
SELECT * FROM DW.VW_PRESCRIPTION_REPORTING ORDER BY LOAD_TIMESTAMP DESC LIMIT 20;
