...
flowchart TD
    ...
    subgraph PIPELINE ["🔄 Data Engineering Pipeline"]
        B["🧹 01–05 Cleaning Notebooks\nstandardize → ingredients\n→ FDA-enrich → trade names → merge"]
        C["📡 Kafka Producer\ntopic: DataDose.in\n(Aiven SASL_SSL)"]
        D["⚡ Databricks PySpark\nStructured Streaming\nNeo4j enrichment + risk scoring"]
    end
...
flowchart LR
    ...
    style S4 fill:#1C1C1E,stroke:#FF3621,color:#E6EDF3
    style S5 fill:#1C1C1E,stroke:#29B5E8,color:#E6EDF3
```
...
```mermaid
sequenceDiagram
    participant CSV as 📂 output_FINAL.csv
    participant PROD as 📡 Kafka Producer
    participant KAFKA as ☁️ Aiven Kafka<br/>(SASL_SSL)
    participant DBX as ⚡ Databricks<br/>PySpark
    participant NEO4J as 🔵 Neo4j AuraDB
    participant SF as ❄️ Snowflake

    CSV->>PROD: Read verified drug records
    PROD->>KAFKA: Publish JSON prescription event<br/>topic: DataDose.in
    KAFKA->>DBX: Structured Streaming consume
    DBX->>NEO4J: Enrich — query INTERACTS_WITH<br/>for each drug in prescription
    NEO4J-->>DBX: Interaction edges + severity scores
    DBX->>DBX: Apply risk scoring function
    DBX->>SF: Write risk-scored rows<br/>(snowflake-spark-connector)
...
flowchart LR
    ...
    FE["⬛ Next.js 15\nRole Dashboard\nReact Flow · Chart.js"] -->|"REST via API Route"| BA["🐍 FastAPI\nmain.py\nAsync Neo4j Driver"]

    BA --> DDI["⚡ DDI Scanner\n/api/scan\nPairwise INTERACTS_WITH\nall drug combinations"]
    BA --> ALT["💊 Alternatives\n/api/alternatives\nCypher traversal\n+ Groq fallback"]
    BA --> TRACE["🔍 Symptom Tracer\n/api/trace-symptom\nMultilingual expansion\nArabic + English"]
    BA --> VIZ["🗺 Visual Map\n/api/visualize-graph\nReact Flow payload\nDDI + TREATS + CAUSES"]
    BA --> OCR["📷 OCR Scanner\n/api/ocr\nGroq Vision LLM\nLLaMA-3.2 11B"]
    BA --> RAG["🤖 GraphRAG\n/api/graphrag\nEntity extraction\n→ Graph retrieval\n→ LLM synthesis"]

    DDI & ALT & TRACE & VIZ --> NEO4J[("🔵 Neo4j AuraDB")]
    OCR & RAG --> GROQ(["🤖 Groq API\nLLaMA-3"])
    RAG --> NEO4J

    style U fill:#1C1C1E,stroke:#00D4FF,color:#E6EDF3
    style NEO4J fill:#1C1C1E,stroke:#008CC1,color:#E6EDF3
...
```mermaid
graph TD
    D1(["💊 Drug A\ne.g. Warfarin"]) -->|"INTERACTS_WITH\n⚠️ severity: HIGH"| D2(["💊 Drug B\ne.g. Aspirin"])
    D1 -->|TREATS| DIS1(["🏥 Disease\ne.g. Atrial Fibrillation"])
    D2 -->|TREATS| DIS2(["🏥 Disease\ne.g. Cardiovascular Risk"])
    D1 -->|CAUSES_REACTION| SYM1(["🔴 Symptom\ne.g. Bleeding"])
    D2 -->|CAUSES_REACTION| SYM2(["🔴 Symptom\ne.g. GI Upset"])

    style D1 fill:#008CC1,stroke:#005E8B,color:#fff
    style D2 fill:#008CC1,stroke:#005E8B,color:#fff
    style DIS1 fill:#059669,stroke:#047857,color:#fff
    style DIS2 fill:#059669,stroke:#047857,color:#fff
    style SYM1 fill:#DC2626,stroke:#B91C1C,color:#fff
    style SYM2 fill:#DC2626,stroke:#B91C1C,color:#fff
```
