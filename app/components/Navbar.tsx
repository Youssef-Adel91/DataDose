...
export default function Navbar() {
  ...
  return (
    <motion.header
      ...
      className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-white/80 border-b border-white/60 shadow-sm"
    >
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="text-sm font-bold text-slate-500 hover:text-teal-600 transition-colors relative group"
            >
              {link.label}
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            ...
            <>
              <Link
                href="/login"
                className="text-sm font-bold text-slate-700 hover:text-teal-600 transition-colors px-4 py-2"
              >
                Log In
              </Link>
              <Link
                href="/login"
                className="text-sm font-bold text-white gradient-teal px-5 py-2.5 rounded-full shadow-md hover:shadow-lg hover:scale-105 transition-all duration-300"
              >
                Get Started
              </Link>
            </>
