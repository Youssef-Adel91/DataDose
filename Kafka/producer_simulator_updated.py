...
# Kafka credentials.
# NOTE: hardcoded as defaults below per request. This file now contains a live
# secret - do not push it to a public repo. env vars still override these defaults.
SASL_USERNAME = get_env("KAFKA_USERNAME", "avnadmin")
SASL_PASSWORD = get_env("KAFKA_PASSWORD", "AVNS_V7suP3ItKX1nEe7zFPu")
...
DATASET_PATH = Path(r"F:\DataDose\DataDose\Data\DataDoseDataset-Cleaned.csv")
...
DOSE_FORMS  = ["cream", "drops", "gel", "injection", "oral_liquid",
               "capsule", "tablet", "ointment", "lotion", "powder",
               "sachet", "spray", "suppository"]
...
def load_drug_pool(csv_path: Path) -> tuple[List[str], dict]:
    """
    ...
    Also builds a drug -> list of observed dosage_form values, so each
    generated prescription can use a form that dataset actually has on
    record for that drug instead of a random one.
    Returns (drug_pool, drug_to_forms).
    """
    ...
    for candidate in ("ingredient_corrected", "activeingredient_clean", "drug_ingredients"):
        if candidate in drug_dataset.columns:
            ingredient_column = candidate
            break

    if ingredient_column is None:
        raise KeyError(
            "None of the expected ingredient columns "
            "('ingredient_corrected', 'activeingredient_clean', 'drug_ingredients') "
            f"were found in {csv_path.name}. Available columns: {list(drug_dataset.columns)}"
        )
    print(f"Using ingredient column: {ingredient_column}")

    has_dosage_form = "dosage_form" in drug_dataset.columns
    if not has_dosage_form:
        print("Warning: no 'dosage_form' column found - falling back to the generic DOSE_FORMS list.")
    ...
    verified_drugs = set()
    drug_to_forms: dict[str, list[str]] = {}

    for _, dataset_row in drug_dataset.iterrows():
        raw_value = dataset_row.get(ingredient_column)
        if pd.isna(raw_value):
            continue
        row_dosage_form = None
        if has_dosage_form:
            form_value = dataset_row.get("dosage_form")
            if pd.notna(form_value) and str(form_value).strip():
                row_dosage_form = str(form_value).strip().lower()

        ingredient_parts = [part.strip() for part in str(raw_value).replace("|", "+").split("+")]
        for ingredient_part in ingredient_parts:
            ...
            if (
                ...
                if row_dosage_form:
                    drug_to_forms.setdefault(normalized_part, []).append(row_dosage_form)

    drug_pool = sorted(verified_drugs)
    drugs_with_forms = sum(1 for d in drug_pool if d in drug_to_forms)
    print(f"Unique drug INNs: {len(drug_pool):,}")
    print(f"Drugs with a known dosage_form: {drugs_with_forms:,} / {len(drug_pool):,}\n")
    return drug_pool, drug_to_forms
...
class PrescriptionGenerator:
    ...
    def __init__(self, drugs: List[str], drug_to_forms: Optional[dict] = None):
        self.drugs = drugs
        self.drug_to_forms = drug_to_forms or {}
    ...
    def next(self) -> dict:
        ...
        # Use a dosage_form actually observed in the dataset for this drug when
        # available, otherwise fall back to the generic DOSE_FORMS pool.
        known_forms = self.drug_to_forms.get(new_drug)
        new_drug_form = random.choice(known_forms) if known_forms else random.choice(DOSE_FORMS)
...
def run(rate: float, dry_run: bool, max_msgs: Optional[int]) -> None:
    ...
    verified_drugs, drug_to_forms = load_drug_pool(DATASET_PATH)
    prescription_generator = PrescriptionGenerator(verified_drugs, drug_to_forms)
    ...
    try:
        while True:
            ...
            if producer:
                try:
                    def _on_error(exc, tx_id=prescription_record["transaction_id"]):
                        nonlocal send_errors
                        send_errors += 1
                        print(f"Kafka delivery failed for tx {tx_id}: {exc}")

                    producer.send(
                        TOPIC,
                        key=prescription_record["transaction_id"],
                        value=prescription_record,
